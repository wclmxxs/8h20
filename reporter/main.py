...
CATALOG_AUTH = os.getenv("REPORT_CATALOG_AUTH", "bernard-edit-bridge-internal-call")
DEPLOYMENT_REPORT_URL = os.getenv("DEPLOYMENT_REPORT_URL", "").strip()
PSM = os.getenv("PSM", "capcut.ai_infra_minimax_h3.dreamina")
SERVICE_ID = os.getenv("SERVICE_ID", "Minimax-H3-Lora-H20")
...
def probe_instance(
    client: httpx.Client, item: dict[str, Any]
    ...
    except Exception as exc:  # noqa: BLE001 - keep reporting through restarts
        error = str(exc)

    slot = int(item["group_index"])
    bernard = {
        "id": item["id"],
        "host": item["host"],
        "ports": [int(item["port"])],
        ...
        "containerInfos": {
            f"h3-8h20-{slot}": {
                "request": {
                    "cpu": int(item.get("cpu", 16)),
                    "memory": int(item.get("memory_mb", 65536)),
                    "nvidia.com/gpu": 8,
                }
            }
        },
...
def report_catalog(
    client: httpx.Client, instances: list[dict[str, Any]]
) -> dict[str, Any]:
    ...
    if payload.get("success") is False:
        raise RuntimeError(
            payload.get("message") or "ReportCatalog returned success=false"
        )
    return payload
...
def report_deployment(
    client: httpx.Client, config: dict[str, Any], details: list[dict[str, Any]]
) -> None:
...
def main() -> None:
    ...
    with httpx.Client(timeout=15) as client:
        while True:
            ...
            except Exception as exc:  # noqa: BLE001 - reporter is a permanent loop
                state["error"] = str(exc)
